...
if {($CurrentProtection == $PS_SECURE)} {
	...
	if {$USE_PSVP == 0} {
		...
		set TestString "Test : CheckSum on wounded row id";
		test_start $TestString;
		...
		set TestString "Test : EraseSector on wounded flash address";
		test_start $TestString;
		set rowID [expr $WOUNDED_FLASH_SIZE/$FLASH_ROW_SIZE] ;
		...
		set TestString "Test : ComputeBasicHash on wounded flash address";
		test_start $TestString;
		set rowID [expr $WOUNDED_FLASH_SIZE/$FLASH_ROW_SIZE] ;
		...
		set TestString "Test : ProgramRow on wounded flash row";
		test_start $TestString;

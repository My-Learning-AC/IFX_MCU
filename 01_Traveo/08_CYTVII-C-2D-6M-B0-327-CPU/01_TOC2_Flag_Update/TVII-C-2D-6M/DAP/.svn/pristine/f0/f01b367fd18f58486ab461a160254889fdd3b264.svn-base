...
if {($CurrentProtection != $PS_NORMAL)} {
    ...
    set nar_ctl [IOR_byte $CYREG_NORMAL_ACCESS_CTL]
    if {$nar_ctl != 1} {
	    puts " This test case pre condition is CYREG_NORMAL_ACCESS_CTL = 1 ";
		puts " Exiting as device does not have the above config: FAIL";
		shutdown
	}
	...
	test_start $TestString;
	#for {set addr 0x10600000} {$addr < 0x10700000} {incr addr 4} {
	#    IOR $addr;
	#	}
	#IOR [expr 0x10630000 -4]
	#set val [IOR [expr $FLASH_START_ADDR + ($MAIN_FLASH_ADVERTIZED_SIZE*7/8) - 4]];
	#if {$val == $READ_ACCESS_FAILED} {
	#    puts "Access failure:FAIL"
	#}
	...
	#set val [IOR [expr $WFLASH_START_ADDR + ($WFLASH_SIZE/2) - 4]];
	#if {$val == $READ_ACCESS_FAILED} {
	#    puts "Access failure:FAIL"
	#}
	test_end $TestString;
	...
	#set val [IOR [expr $SFLASH_START_ADDR + ($SFLASH_SIZE/2) - 4]];
	#if {$val == $READ_ACCESS_FAILED} {
	#    puts "Access failure:FAIL"
	#}
	test_end $TestString;
	...
	#set val [IOR [expr $SRAM_START_ADDR + ($SRAM0_SIZE/8*7) - 4]];
	#if {$val == $READ_ACCESS_FAILED} {
	#    puts "Access failure:FAIL"
	#}
	test_end $TestString;
	...
	if {$DUT == $TVIIC2D4M_SILICON || $DUT == $TVIIC2D4M_PSVP} {
		#Do nothing as there is no CM7_1 core

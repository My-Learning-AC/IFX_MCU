...
proc acquire_TestMode_SROM {} {
	...
	global INIT_ACQUIRE_COUNT DUT DUT_NAME DUT; 
	global TVIIBE1M_SILICON TVIIBE2M_SILICON TVIIBE4M_SILICON TVIICE4M_SILICON TVIIC2D4M_SILICON TVIIC2D6M_SILICON TVIIC2D6MDDR_SILICON TVIIBH4M_SILICON TVIIBH8M_SILICON;
	global TVIIBE1M_PSVP TVIIBE2M_PSVP TVIIBE4M_PSVP TVIICE4M_PSVP TVIIC2D4M_PSVP TVIIC2D6M_PSVP TVIIC2D6MDDR_PSVP TVIIBH4M_PSVP TVIIBH8M_PSVP;
	...
	if {$INIT_ACQUIRE_COUNT == 0} {
		...
		if {$DUT == $TVIIBE1M_SILICON} {
			...
			source [find target/traveo2_6m_b0.cfg]
		} elseif {$DUT == $TVIIC2D6MDDR_SILICON} {
			source [find target/traveo2_6m_b0.cfg]
		} elseif {$DUT == $TVIIBH4M_SILICON} {
			source [find target/traveo2_4m.cfg]
		} elseif {$DUT == $TVIIBH8M_SILICON} {
			source [find target/traveo2_8m_b0.cfg]
		} elseif {$DUT == $TVIIBE1M_PSVP} {
			source [find target/traveo2_1m_a0.cfg]
		} elseif {$DUT == $TVIIBE2M_PSVP} {
			source [find target/traveo2_2m.cfg]
		} elseif {$DUT == $TVIIBE4M_PSVP} {
			source [find target/traveo2_be_4m.cfg]
		} elseif {$DUT == $TVIICE4M_PSVP} {
			source [find target/traveo2_ce_4m_psvp.cfg]
		} elseif {$DUT == $TVIIC2D4M_PSVP} {
			source [find target/traveo2_c2d_4m_psvp.cfg]
		} elseif {$DUT == $TVIIC2D6M_PSVP} {
			source [find target/traveo2_6m_psvp.cfg]
		} elseif {$DUT == $TVIIC2D6MDDR_PSVP} {
			source [find target/traveo2_6m_psvp.cfg]
		} elseif {$DUT == $TVIIBH4M_PSVP} {
			source [find target/traveo2_4m.cfg]
		} elseif {$DUT == $TVIIBH8M_PSVP} {
			source [find target/traveo2_8m_psvp_b.cfg]
		} else {
			puts "Invalid Device choice"
		}
...
proc checkTrim {} {
	...
	IOR $MPU15_MS_CTL 0;
	set trimTableLength [[IOR_byte [expr $TRIM_TABLE_LEN_ADDR + 1]]<<8] + [IOR_byte $TRIM_TABLE_LEN_ADDR];
...
proc GetTrimAddrInSflash {trimRegisterAddr} {
	...
	set trimTableLength [expr [IOR_byte $TRIM_TABLE_LEN_ADDR] + [expr [IOR_byte [expr $TRIM_TABLE_LEN_ADDR + 1]]<<8]];
	puts "TRIM TABLE LENGTH: $trimTableLength"
	set total_cases  [expr $trimTableLength - 0x3];
	...
	while {$byteAddr <= ($TRIM_START_ADDR + $trimTableLength - 0x3)} {
		...
		set addrFormat    [expr ($curCmdValue & 0xC0)>>6];
		#puts "Cmd Tuple : (NoOfValues : TxrSize : addrFormat) == $numOfValues : $txrSize : $addrFormat";
		#puts "Previous Byte Addr: $byteAddr"
		incr byteAddr;
		#puts "Updated Byte Addr: $byteAddr"
...
proc ReadFaultRegisters {} {
	...
	#STATUS
	set status [IOR 0x4021000C];

	puts "\nFault DATA register\n";
	#DATA
	set data0 [IOR 0x40210010];
	set data1 [IOR 0x40210014];
	set data2 [IOR 0x40210018];
	set data3 [IOR 0x4021001C];

	puts "\nFault Pending register\n";
	#Pending
	set pending0 [IOR 0x40210040];
	set pending1 [IOR 0x40210044];
	set pending2 [IOR 0x40210048];
	set pending3 [IOR 0x4021004C];

	puts "\nRAM0_CTL0 ECC_CHECK_DIS(bit 19 should be 0)";
	IOR $CYREG_CPUSS_RAM0_CTL0;
	puts "\n\n";
	
	if {[expr $status & 0x80000000] == 0x80000000} {
		puts "\nFault has occured!!\n"
		set fault [expr $status & 0x000000FF];
		if {$fault == 0} {
			puts "BUS MASTER 0 MPU/SMPU FAULT OCCURED!!"
		} elseif {$fault == 1} {
			puts "BUS MASTER 1 MPU/SMPU FAULT OCCURED!!"
		} elseif {$fault == 2} {
			puts "BUS MASTER 2 MPU/SMPU FAULT OCCURED!!"
		} elseif {$fault == 3} {
			puts "BUS MASTER 3 MPU/SMPU FAULT OCCURED!!"
		} elseif {$fault == 4} {
			puts "BUS MASTER 4 MPU/SMPU FAULT OCCURED!!"
		} elseif {$fault == 5} {
			puts "BUS MASTER 5 MPU/SMPU FAULT OCCURED!!"
		} elseif {$fault == 6} {
			puts "BUS MASTER 6 MPU/SMPU FAULT OCCURED!!"
		} elseif {$fault == 7} {
			puts "BUS MASTER 7 MPU/SMPU FAULT OCCURED!!"
		} elseif {$fault == 8} {
			puts "BUS MASTER 8 MPU/SMPU FAULT OCCURED!!"
		} elseif {$fault == 9} {
			puts "BUS MASTER 9 MPU/SMPU FAULT OCCURED!!"
		} elseif {$fault == 10} {
			puts "BUS MASTER 10 MPU/SMPU FAULT OCCURED!!"
		} elseif {$fault == 11} {
			puts "BUS MASTER 11 MPU/SMPU FAULT OCCURED!!"
		} elseif {$fault == 12} {
			puts "BUS MASTER 12 MPU/SMPU FAULT OCCURED!!"
		} elseif {$fault == 13} {
			puts "BUS MASTER 13 MPU/SMPU FAULT OCCURED!!"
		} elseif {$fault == 14} {
			puts "BUS MASTER 14 MPU/SMPU FAULT OCCURED!!"
		} elseif {$fault == 15} {
			puts "BUS MASTER 15 MPU/SMPU FAULT OCCURED!!"
		} elseif {$fault == 16} {
			puts "CM7_1 TCM CORRECTABLE ECC FAULT OCCURED!!"
		} elseif {$fault == 17} {
			puts "CM7_1 TCM NON-CORRECTABLE ECC FAULT OCCURED!!"
		} elseif {$fault == 18} {
			puts "CM7_0 CACHE CORRECTABLE ECC FAULT OCCURED!!"
		} elseif {$fault == 19} {
			puts "CM7_0 CACHE NON-CORRECTABLE ECC FAULT OCCURED!!"
		} elseif {$fault == 20} {
			puts "CM7_1 CACHE CORRECTABLE ECC FAULT OCCURED!!"
		} elseif {$fault == 21} {
			puts "CM7_1 CACHE NON-CORRECTABLE ECC FAULT OCCURED!!"
		} elseif {$fault == 25} {
			puts "PERIPHERAL INTERFACE MASTER-4 FAULT OCCURED!!"
		} elseif {$fault == 26} {
			puts "PERI CORRECTABLE ECC FAULT OCCURED!!"
		} elseif {$fault == 27} {
			puts "PERI NON-CORRECTABLE ECC FAULT OCCURED!!"
		} elseif {$fault == 28} {
			puts "PERIPHERAL INTERFACE MASTER-0 FAULT OCCURED!!"
		} elseif {$fault == 29} {
			puts "PERIPHERAL INTERFACE MASTER-1 FAULT OCCURED!!"
		} elseif {$fault == 30} {
			puts "PERIPHERAL INTERFACE MASTER-2 FAULT OCCURED!!"
		} elseif {$fault == 31} {
			puts "PERIPHERAL INTERFACE MASTER-3 FAULT OCCURED!!"
		} elseif {$fault == 32} {
			puts "PERIPHERAL GROUP-0 FAULT OCCURED!!"
		} elseif {$fault == 33} {
			puts "PERIPHERAL GROUP-1 FAULT OCCURED!!"
		} elseif {$fault == 34} {
			puts "PERIPHERAL GROUP-2 FAULT OCCURED!!"
		} elseif {$fault == 35} {
			puts "PERIPHERAL GROUP-3 FAULT OCCURED!!"
		} elseif {$fault == 36} {
			puts "PERIPHERAL GROUP-4 FAULT OCCURED!!"
		} elseif {$fault == 37} {
			puts "PERIPHERAL GROUP-5 FAULT OCCURED!!"
		} elseif {$fault == 38} {
			puts "PERIPHERAL GROUP-6 FAULT OCCURED!!"
		} elseif {$fault == 39} {
			puts "PERIPHERAL GROUP-7 FAULT OCCURED!!"
		} elseif {$fault == 40} {
			puts "PERIPHERAL GROUP-8 FAULT OCCURED!!"
		} elseif {$fault == 41} {
			puts "PERIPHERAL GROUP-9 FAULT OCCURED!!"
		} elseif {$fault == 42} {
			puts "PERIPHERAL GROUP-10 FAULT OCCURED!!"
		} elseif {$fault == 43} {
			puts "PERIPHERAL GROUP-11 FAULT OCCURED!!"
		} elseif {$fault == 44} {
			puts "PERIPHERAL GROUP-12 FAULT OCCURED!!"
		} elseif {$fault == 45} {
			puts "PERIPHERAL GROUP-13 FAULT OCCURED!!"
		} elseif {$fault == 46} {
			puts "PERIPHERAL GROUP-14 FAULT OCCURED!!"
		} elseif {$fault == 47} {
			puts "PERIPHERAL GROUP-15 FAULT OCCURED!!"
		} elseif {$fault == 48} {
			puts "MAIN FLASH BUS ERROR FAULT OCCURED!!"
		} elseif {$fault == 49} {
			puts "MAIN FLASH ECC FAULT OCCURED!!"
		} elseif {$fault == 50} {
			puts "MAIN FLASH NON-CORRECTABLE ECC FAULT OCCURED!!"
		} elseif {$fault == 51} {
			puts "WORK FLASH BUS ERROR FAULT OCCURED!!"
		} elseif {$fault == 52} {
			puts "WORK FLASH ECC FAULT OCCURED!!"
		} elseif {$fault == 53} {
			puts "WORK FLASH NON-CORRECTABLE ECC FAULT OCCURED!!"
		} else {
		...
		puts "Data 2: $data2"
		puts "Data 3: $data3"
	} elseif {($pending0 != 0) || ($pending1 != 0) || ($pending2 != 0) || ($pending3 != 0)} {
		puts "\nPending Faults!!\n"
		puts "Pending 0 Register: $pending0"
		puts "Pending 1 Register: $pending1"
		puts "Pending 2 Register: $pending2"
		puts "Pending 3 Register: $pending3"
	} else {
		IOW 0x4021000C 0x0;
		IOR 0x4021000C;
		puts "No Faults!!"

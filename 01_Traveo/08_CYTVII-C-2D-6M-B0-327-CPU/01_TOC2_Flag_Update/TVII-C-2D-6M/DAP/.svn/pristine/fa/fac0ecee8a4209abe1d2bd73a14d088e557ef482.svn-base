...
proc SROM_Checksum {sysCallType rowID wholeFlash FlashType bank} {
	...
	global DUT TVIIBH4M_SILICON TVIIBE4M_SILICON

	puts "SROM_Checksum API: Start"

	if {$DUT >= $TVIIBE4M_SILICON} {
	
	    set value [expr ($CHECKSUM_OPC<<$OPC_SHIFT) + ($FlashType<<22)+($wholeFlash<<21) + ($rowID<<4) +($bank<<1)];
	} else {
	    set value [expr ($CHECKSUM_OPC<<$OPC_SHIFT) + ($FlashType<<22)+($wholeFlash<<21) + ($rowID<<8) + ($bank<<7)];
	}
	if {$sysCallType == $SYS_CALL_GREATER32BIT} {
        SYSCALL_GreaterThan32bits_Alt $value;
	} else {
...
proc SROM_DirectExecute {sysCallType funcType funcAddress argument return addrRegionType} {
	...
	if {$sysCallType == $SYS_CALL_GREATER32BIT} {
		...
		IOW [expr $SRAM_SCRATCH + 0x0C] $return;
		SYSCALL_GreaterThan32bits_Alt $value;
	} else {
...
proc SYSCALL_LessThan32bits_Alt {data} {
	...
	global CYREG_IPC_STRUCT_ACQUIRE  CYREG_IPC_STRUCT_RELEASE CYREG_IPC_STRUCT_NOTIFY CYREG_IPC_STRUCT_DATA CYREG_IPC_STRUCT_DATA1 CYREG_IPC_STRUCT_LOCK_STATUS CYREG_IPC_STRUCT_DATA_RMA;
	
	
	set cyreg_ipc_acquire $CYREG_IPC_STRUCT_ACQUIRE ;
	set cyreg_ipc_data $CYREG_IPC_STRUCT_DATA;
	set cyreg_ipc_notify $CYREG_IPC_STRUCT_NOTIFY;
...
proc Srom_ReturnCheck {IsArgInIPCData} {
	...
	set cyreg_ipc_data $CYREG_IPC_STRUCT_DATA;
	set cyreg_ipc_lock_status $CYREG_IPC_STRUCT_LOCK_STATUS;
...
#set addr [expr 0x40010800 + [IOR_byte 0x17007044] * 0x40];
#puts [format "Fixed PPU-[IOR_byte 0x17007044]"];
#puts [format "-----------------------------------------------------------------------"];
...
#puts [format "-----------------------------------------------------------------------"];
#
#set addr [expr 0x40010800 + [IOR_byte 0x17007048] * 0x40];
#puts [format "Fixed PPU-[IOR_byte 0x17007048]"];
#puts [format "-----------------------------------------------------------------------"];
#puts [format "Slave Address"];
#IOR [expr $addr + 0x0];
#puts [format "Slave Size"];
...
#puts [format "-----------------------------------------------------------------------"];
#
#set addr [expr 0x40010800 + [IOR_byte 0x1700704C] * 0x40];
#puts [format "Fixed PPU-[IOR_byte 0x1700704C]"];
#puts [format "-----------------------------------------------------------------------"];
#puts [format "Slave Address"];
#IOR [expr $addr + 0x0];
#puts [format "Slave Size"];

...
if {($CurrentProtection != $PS_SECURE)} {
	...
	#test_start $TestString;
	#test_compare $READ_ACCESS_FAILED [IOR [expr $SFLASH_START_ADDR + ($FLASH_ROW_SIZE*$SFLASH_NAR_ROW_IDX)]];
	#test_end $TestString;
    #
	#set TestString "Test FUNC_111c: Read NDAR ";
	#test_start $TestString;
	#test_compare $READ_ACCESS_FAILED [IOR [expr $SFLASH_START_ADDR + ($FLASH_ROW_SIZE*$SFLASH_NAR_ROW_IDX) +0x4]];
	#test_end $TestString;
}

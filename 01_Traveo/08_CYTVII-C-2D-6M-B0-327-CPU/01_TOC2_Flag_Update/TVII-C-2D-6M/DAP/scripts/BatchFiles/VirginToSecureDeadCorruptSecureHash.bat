...
set LOGPATH="Report\SECURE_DEAD_CORRUPT_HASH_%SUBFILENAME%"

if not exist "%LOGPATH%" mkdir "%LOGPATH%"

%OPEN_OCD% -f HelperScripts\ReadSflash.tcl > %LOGPATH%\1_Sflash.txt
@rem %OPEN_OCD% -f HelperScripts\Program_FB_8M_B0_Si.tcl >%LOGPATH%\2_Program_FB_8M_B0_Si.txt
...
%OPEN_OCD% -f HelperScripts\ProvisionedToSecureCorruptSecureHash.tcl > %LOGPATH%\10_ProvisionedToSecureCorruptSecureHash.txt
%OPEN_OCD% -f HelperScripts\BasicTest.tcl > %LOGPATH%\13_BasicTest.txt
%OPEN_OCD% -f CQ_SCRIPTS\SECURE_DEAD\TestSecureDeadAcessRestrictions.tcl > %LOGPATH%\14_TestSecureAcessRestrictions.txt
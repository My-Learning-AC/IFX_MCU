...
if {($CurrentProtection == $PS_VIRGIN)} {
	...
	set TestString " Read & Write first word 2KB of SRAM which is  protected";
	test_start $TestString;
	...
	set TestString " Read & Write last word 2KB of SRAM which is  protected";
	test_start $TestString;
	IOW [expr $SRAM_START_ADDR + $SIZE_2KB -4] 0xDEADBEEF;
	IOR [expr $SRAM_START_ADDR + $SIZE_2KB -4];
	#Expect a hardfault in step IOW [expr $SRAM_START_ADDR] 0xDEADBEEF; and to be automated
	ReadFaultRegisters;
}
Log_Post_Test_Check;

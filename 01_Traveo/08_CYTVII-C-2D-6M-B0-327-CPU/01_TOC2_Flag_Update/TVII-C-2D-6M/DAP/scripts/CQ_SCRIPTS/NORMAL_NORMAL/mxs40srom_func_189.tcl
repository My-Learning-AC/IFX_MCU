...
if {$protState == $PS_NORMAL} {
	...
	puts [format "Region Address 0x%08x " $region_addr];
	...
	if {$PROGRAM_PATCH_AVAILABLE == 0x1} {
		for {set data_width $DATA_WIDTH_32BITS} {$data_width <= $DATA_WIDTH_4096BITS} { incr data_width} {			
			...
			test_start $TestString;
			test_compare $STATUS_INVALID_ARGUMENT_LOCATION [SROM_ProgramRow_Work $SYS_CALL_GREATER32BIT $CM0P_BLOCKING $SKIP_BLANK_CHECK_EN $data_width $DATA_LOC_SRAM $FM_INTR_MASK_RESET $WFLASH_START_ADDR $userrow];
			test_end $TestString;			
		}		
	} else {

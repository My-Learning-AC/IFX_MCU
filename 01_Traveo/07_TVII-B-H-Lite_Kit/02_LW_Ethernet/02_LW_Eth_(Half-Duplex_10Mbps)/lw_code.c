...
#include "lw_eth_drivers/lw_eth.h"


/********************************************************/
// Speed & Duplex Configuration

#define HALF_DUPLEX_10MBPS          0
#define FULL_DUPLEX_100MBPS         1
#define SPEED_N_DUPLEX_MODE         FULL_DUPLEX_100MBPS              // Configure here the duplex and speed

#if SPEED_N_DUPLEX_MODE == HALF_DUPLEX_10MBPS
    static stc_en_eth_link_speed_t speed    =   LW_ETH_10MBPS;
    #define FULL_DUPLEX                         false               // true = Full-Duplex, false = Half-Duplex
    
#elif SPEED_N_DUPLEX_MODE == FULL_DUPLEX_100MBPS
    static stc_en_eth_link_speed_t speed    =   LW_ETH_100MBPS;
    #define FULL_DUPLEX                         true               // true = Full-Duplex, false = Half-Duplex
#endif
...
int main(void)
{
    ...
    Lw_Eth_InitEther(interface_mode,speed,FULL_DUPLEX,&ETH0Config);         // Initializes the Ethernet
...
static void InitPHY_Dp83825i (void) {
    ...
    // for checking Half-Duplex/10Mbps mode,
    // If traped into this while loop then change the configuration
    // in your PC as Half-Duplex/10Mbps in network adapter
    do
    {
    u16PHYSTS = Lw_Eth_PhyRead(DP83825I_MDIO_PHYSTS, PHY_ADDR, &ETH0Config);
    } while(!((u16PHYSTS & 6) == 2));

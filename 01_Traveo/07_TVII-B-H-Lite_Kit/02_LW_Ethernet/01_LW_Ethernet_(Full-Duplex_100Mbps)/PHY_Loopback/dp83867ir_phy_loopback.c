/* This example is used to test the Loopback Mode of PHY*/
/***************************************************************************//**
* \file main_cm7_0.c
*
* \version 1.0
...
{
    Cy_EthIf00_DecodeEvent(); //Decodes the Interrupt event
}
...
#define ETH_FRAME_SIZE              1500   //Excluding FCS,DMAC,SMAC,SFD,PREAMBLE 
...
uint16_t rx_count = 1,tx_count = 1;
/********************************************************/
...
int main(void) {
    ...
    while(tx_count < MAX_PACKETS) {
          while(CY_ETHIF_SUCCESS != Cy_EthIf_TransmitFrame(ETH_REG_BASE, u8DummyTxBuf, ETH_FRAME_SIZE, CY_ETH_QS0_0, bLastBuffer));
...
static void InitPHY_DP83867IR (void) {
    ...
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x1F, 0x8000, PHY_ADDR); //SW_RESET Bit[15] 
    ...
    Cy_EthIf_PhyRegWrite(ETH_REG_BASE, 0x16, 0x0004, PHY_ADDR);             /** BISCR   */  

...
#define LED0_PORT     GPIO_PRT6
#define LED0_PIN      3
#define LED0_PIN_MUX  P6_3_GPIO
...
int main(void)
{
    ...
    stcMsg.idConfig.extended = false;  // 11 bit Identifier //
    stcMsg.idConfig.identifier = 0x525;
    stcMsg.dataConfig.dataLengthCode = 15;  // 64 byte data length //
...
void CAN_RxMsgCallback(bool bRxFifoMsg, uint8_t u8MsgBufOrRxFifoNum, cy_stc_canfd_msg_t* pstcCanFDmsg)
{
    /* Just loop back to the sender with +1 ID */  // 888888888888 the linking 8888888888888888888 //
   if(((pstcCanFDmsg->dataConfig.data[0]) & 0xFF) == 0x57)
   Cy_GPIO_Inv(LED0_PORT, LED0_PIN);
  
  //Term_Printf("\n\r%X", bRxFifoMsg);
  //Term_Printf("\n\r%X", u8MsgBufOrRxFifoNum);
  
  for(volatile uint8_t i=0; i<16; i++)
    Term_Printf("\n\r%#X", pstcCanFDmsg->dataConfig.data[i]);
  
  
  
  
    pstcCanFDmsg->idConfig.identifier += 1u;
    Cy_CANFD_UpdateAndTransmitMsgBuffer(CY_CANFD_TYPE, 0u, pstcCanFDmsg);
}

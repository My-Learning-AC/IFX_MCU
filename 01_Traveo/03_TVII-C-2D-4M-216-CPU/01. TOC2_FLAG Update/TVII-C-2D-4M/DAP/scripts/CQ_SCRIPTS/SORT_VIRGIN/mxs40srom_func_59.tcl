...
puts [format "\nProtection State(expected SECURE DEBUG) = 0x%08x \n" $current_life_Cycle_stage];   
...
set result [SROM_TransitionToSecure $SYS_CALL_GREATER32BIT $BLOW_S_FUSE $SecureAccessRestrictions $DeadAccessRestrictions];

if {$result != $STATUS_INVLAID_PROTECTION} {
	puts "Test Case PASS"
} else {
	puts "SecureDebug to Secure transition not allowed, FAIL"
}

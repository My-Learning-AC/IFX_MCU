...
if {$protState == $PS_NORMAL} {
	...
	UpdateAppProt $mFlash_sl_addr $mFlash_sl_size $mFlash_sl_att $mFlash_ms_att \
				  $wmFlash_sl_addr $wFlash_sl_size $wFlash_sl_att $wFlash_ms_att \
	...
	if {$PROGRAM_PATCH_AVAILABLE == 0x1} {
		for {set data_width $DATA_WIDTH_32BITS} {$data_width <= $DATA_WIDTH_4096BITS} { incr data_width} {			
			...
			test_start $TestString;
			test_compare $STATUS_ROW_PROTECTED [SROM_ProgramRow_Work $SYS_CALL_GREATER32BIT $CM0P_BLOCKING $SKIP_BLANK_CHECK_EN $data_width $DATA_LOC_SRAM $FM_INTR_MASK_RESET $WFLASH_START_ADDR $userrow];
			test_end $TestString;

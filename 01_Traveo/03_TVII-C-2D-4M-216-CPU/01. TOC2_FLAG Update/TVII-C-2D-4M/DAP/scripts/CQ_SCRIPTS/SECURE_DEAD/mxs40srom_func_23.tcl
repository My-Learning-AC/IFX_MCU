...
if {$USE_PSVP == 0} {
	...
	#------------------------------------------------------------------
	set TestString "Test FUNC_23: EraseAll";
	...
	set TestString "Test FUNC_23c: ProgramRow: MAIN FLASH 1st row";
	test_start $TestString;
	...
	set TestString "Test FUNC_23c: ProgramRow: MAIN FLASH last row";
	test_start $TestString;
	test_compare $STATUS_SUCCESS [SROM_ProgramRow $SYS_CALL_GREATER32BIT $CM0P_BLOCKING $SKIP_BLANK_CHECK_EN $DATA_WIDTH_4096BITS $DATA_LOC_SARM $FM_INTR_MASK_RESET [expr $FLASH_START_ADDR + $FLASH_SIZE - 4] $userrow];
	test_end $TestString;
	...
	puts "Test case to be executed for silicon only".
}

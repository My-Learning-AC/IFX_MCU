...
for {set data_width $DATA_WIDTH_32BITS} {$data_width <= $DATA_WIDTH_32BITS} { incr data_width} {
		...
		test_start $TestString;
		test_compare $STATUS_SUCCESS [SROM_EraseSector $SYS_CALL_GREATER32BIT $CM0P_BLOCKING [expr $WFLASH_START_ADDR + 0x800] $FM_INTR_MASK_RESET];
		test_end $TestString;

		set TestString "Test : ProgramRow: WORK FLASH LG Sector";
		test_start $TestString;
		test_compare $STATUS_SUCCESS [SROM_ProgramRow $SYS_CALL_GREATER32BIT $CM0P_BLOCKING $SKIP_BLANK_CHECK_EN $data_width $DATA_LOC_SRAM $FM_INTR_MASK_RESET [expr $WFLASH_START_ADDR + 0x800] $userrow];
		test_end $TestString;
		...
		for {set i 0} {$i < [expr $numBytesProg/4]} {incr i} {
			...
			test_start $TestString;
			test_compare [lindex $userrow $i] [IOR [expr $WFLASH_START_ADDR + 0x800 + $i*4]];
			test_end $TestString;
		}
...
if {$PROGRAM_PATCH_AVAILABLE == 0x1} {
	for {set data_width 3} {$data_width <= $DATA_WIDTH_4096BITS} { incr data_width} {
		...
		set TestString "Test : ProgramRow: WORK FLASH LG Sector";
		test_start $TestString;
		test_compare $STATUS_SUCCESS [SROM_ProgramRow_Work $SYS_CALL_GREATER32BIT $CM0P_BLOCKING $SKIP_BLANK_CHECK_EN $data_width $DATA_LOC_SRAM $FM_INTR_MASK_RESET [expr $WFLASH_START_ADDR + 0x800] $userrow];
		test_end $TestString;

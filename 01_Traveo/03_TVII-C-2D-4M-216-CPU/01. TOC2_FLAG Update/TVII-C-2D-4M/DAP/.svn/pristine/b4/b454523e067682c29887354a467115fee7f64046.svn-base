...
test_start $TestString;
test_compare [expr $STATUS_SUCCESS | [GetExpectedHash $FLASH_START_ADDR [expr $FLASH_ROW_SIZE - 1]]] [SROM_ComputeBasicHash $SYS_CALL_GREATER32BIT $FLASH_START_ADDR [expr $FLASH_ROW_SIZE - 1] $BASIC_HASH];
test_end $TestString;
...
if {$USE_PSVP == 0} {
	...
	set TestString "Test : ComputeBasicHash";
	test_start $TestString;
	test_compare $STATUS_SUCCESS [expr 0xFFFFFF00 & [SROM_ComputeBasicHash $SYS_CALL_GREATER32BIT $FLASH_START_ADDR [expr $FLASH_SIZE - 1] $BASIC_HASH]];
	test_end $TestString;

	set rowID 0x0;
	set TestString "Test : ComputeBasicHash";
	test_start $TestString;
	test_compare $STATUS_SUCCESS  [expr 0xFFFFFF00 & [SROM_ComputeBasicHash $SYS_CALL_GREATER32BIT $FLASH_START_ADDR [expr $FLASH_SIZE - 1] $CRC8SAE]];
	test_end $TestString;

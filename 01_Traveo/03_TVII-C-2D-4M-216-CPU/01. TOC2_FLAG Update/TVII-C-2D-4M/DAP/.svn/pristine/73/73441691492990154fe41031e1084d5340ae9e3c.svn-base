...
#Transit_NormalPov_To_Secure $BLOW_SD_FUSE $SecureAccessRestrictions $DeadAccessRestrictions;

# Uncomment this for Transition to Secure Mode
Transit_NormalPov_To_Secure $BLOW_S_FUSE $SecureAccessRestrictions $DeadAccessRestrictions;
...
# =begin
# puts "\nRead P Fuse Bits to validate correct programming of fuses...\n";
# #Read B Bits
# set byte_addr 0x0;
# set result [Read_Fuse_Byte $SYS_CALL_LESS32BIT $byte_addr];
# if {$result != ($STATUS_SUCCESS + 0xC0  +0x29)} {
#    incr fail_count;
#    puts [format "\nBlow B Bits 0x%08x: FAIL!!\n" $result];	
# } else {
#    puts [format "\nBlow B Bits 0x%08x: PASS!!\n" $result];	
# }
# =cut
...
# =begin
# if {$protection_state != 0x02} {
#     incr fail_count;
# 	puts "\nProtection state is not NORMAL\n";	
# }
# =cut

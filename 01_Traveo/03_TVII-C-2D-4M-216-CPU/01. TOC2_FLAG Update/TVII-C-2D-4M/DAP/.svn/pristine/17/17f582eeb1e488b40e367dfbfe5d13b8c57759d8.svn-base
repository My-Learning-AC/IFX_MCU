...
if {($CurrentProtection == $PS_VIRGIN) || ($CurrentProtection == $PS_NORMAL)} {
	...
	#set func_addr_typ0_flash [expr (0x2800101d & 0x3FFFFF)];
	set func_addr_typ0_flash 0x10001a75;
	...
	set TestString "1. Test FUNC_35: DIRECTEXECUTE API: Code in FLASH Tpe0";
	...
	set TestString "2. Test FUNC_35: After function call the variable should be incremented by 1";
	test_start $TestString;
	...
	test_end $TestString;
shutdown;
	set TestString "3. Test FUNC_35: DIRECTEXECUTE API: Code in FLASH Tpe0";
	test_start $TestString;
	SROM_DirectExecute $api_call_type $func_type $addr $argument $return $code_placed;
	test_end $TestString;


	set TestString "4. Test FUNC_35: Evaluate if function got executed and updated variable in SRAM";
	test_start $TestString;
	set val2 [IOR $dirExecVariable];

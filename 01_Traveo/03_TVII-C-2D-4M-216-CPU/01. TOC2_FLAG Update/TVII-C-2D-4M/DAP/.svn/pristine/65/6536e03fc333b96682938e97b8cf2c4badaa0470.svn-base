...
if {$CurrentProtection != $PS_VIRGIN} {
   puts " Not valid for protection state other than VIRGIN: FAIL"
} else {
	...
	set TestString "Test: UPDATE UID in SFLASH";
	...
	set TestString "Test: Corrupt PKEY format by programming UID in PKEY row";
	test_start $TestString;
	test_compare 0xa0000000 [SROM_WriteRow $SYS_CALL_GREATER32BIT 0 $PKEY_START_ADDR 0 $UID];		
	test_end $TestString;
	
	set TestString "Test: UPDATE TOC2 To add PKEY address";
	test_start $TestString;
	...
	set TestString "Test: OpenRma API in VIRGIN Protection State";
	#Fixed Magic Number
	set cmd_id 0x120029F0;
	#set unique_id {0x01947fb4 0x02032332 0x00000000};

	#Random Unique Id(Invalid) to be passed to api
	...
	test_compare $STATUS_INVALID_PUBLIC_KEY [SROM_OpenRMA $SYS_CALL_VIA_SRAM_SCRATCH $lenOfObjects $cert $signWord];
	test_end $TestString;

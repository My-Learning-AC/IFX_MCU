...
IOW 0x40100050 0xFFFFFFFF;
IOW 0x40100054 0xFFFFFFFF;
IOW 0x40100058 0xFFFFFFFF;
IOR 0x4022000C;
IOR 0x4022002C;
IOR 0x4022004C;
IOR 0x4022006C;
...
if {($CurrentProtection != $PS_SECURE && $CurrentProtection != $PS_NORMAL)} {
    puts " This test case is not meant for protection state other than NORMAL & SECURE: FAIL";
} else {
	...
	set unique_id [list 0x01947fb4 0x02032439 0x00000000]; #Correct UID -> [list 0x01947fb4 0x02032439 0x00000000];
	...
	#IOW 0x4010000C 0x0;
	ReadFaultRegisters;
	test_compare $STATUS_SUCCESS [SROM_TransitionToRMA $SYS_CALL_GREATER32BIT $lenOfObjects $cert $signWord];
	ReadFaultRegisters;
	...
	SROM_SiliconID $SYS_CALL_LESS32BIT 1;
}
set FlowEndTime [clock seconds];

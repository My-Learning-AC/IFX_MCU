...
set TestString "Test FUNC_67: Read PC value, it should be a MAIN FLASH address if an app is programmed";
test_start $TestString;
set pcVal [ReadPC];
puts "Program Counter is $pcVal"
#test_compare 0x00000344 [ReadPC];
if {($pcVal > $FLASH_START_ADDR) && ($pcVal < [expr $FLASH_START_ADDR + $FLASH_SIZE])} {
    puts "DEVICE IN MAIN: PASS";
} else {
    puts "DEVICE IN MAIN: FAIL";
}
test_end $TestString;

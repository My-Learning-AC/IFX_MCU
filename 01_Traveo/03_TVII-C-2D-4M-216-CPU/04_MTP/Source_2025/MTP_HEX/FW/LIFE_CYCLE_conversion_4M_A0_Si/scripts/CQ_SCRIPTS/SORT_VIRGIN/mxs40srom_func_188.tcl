...
if {$protState == $PS_NORMAL} {
	...
	if {$PROGRAM_PATCH_AVAILABLE == 0x1} {
		for {set data_width $DATA_WIDTH_32BITS} {$data_width <= $DATA_WIDTH_4096BITS} { incr data_width} {			
			...
			test_start $TestString;
			test_compare $STATUS_NVM_PROTECTED [SROM_ProgramRow_Work $SYS_CALL_GREATER32BIT $CM0P_BLOCKING $SKIP_BLANK_CHECK_EN $data_width $DATA_LOC_SRAM $FM_INTR_MASK_RESET $wFlash_sl_addr $userrow];
			test_end $TestString;

...
if {($CurrentProtection != $PS_VIRGIN)} {
	...
	#This restore in case if the hash is already corrupted.
	set TestString "Test  SROM_Calibrate: Restore Hash on trims and call Calibrate API";
	test_start $TestString;
	test_compare $STATUS_SUCCESS [RestoreCorrectHashOnTrims];
	test_end $TestString;
	...
	test_start $TestString;
	test_compare $STATUS_SUCCESS [SROM_Calibrate $SYS_CALL_GREATER32BIT 0x0 0x0];
	test_end $TestString;



	set TestString "Test  Corrupt Hash on Trims";
	test_start $TestString;
	test_compare $STATUS_SUCCESS [CorruptHashOnTrims];
	test_end $TestString;

	set TestString "Test  SROM_Calibrate: When hash on trims is corrupted";
	test_start $TestString;
	test_compare $STATUS_HASH_FAILED [expr 0xF1000000 & [SROM_Calibrate $SYS_CALL_GREATER32BIT 0x0 0x0]];
	test_end $TestString;

...
if {($CurrentProtection != $PS_NORMAL)} {
	...
	set TestString "Test FUNC_111a: WriteRow on NAR ROW when NORMAL_ACCESS_CTL.WIDEN_ENABLE = 1 ";
	...
	IOR 0x17001A00;
	IOR 0x17001A04;
	#Reset Acquire for ROM boot to apply restrictions
	re_boot;
	acquire_TestMode_SROM;
	#shutdown
	...
	test_start $TestString;
	test_compare $READ_ACCESS_FAILED [IOR [expr $WFLASH_START_ADDR + ($WFLASH_SIZE/2) +4]]; #This will cause openocd to crash. uncomment test and comment to test next
	#test_compare 0x0 [IOR [expr $WFLASH_START_ADDR + ($WFLASH_SIZE/2) - 4]];
	test_end $TestString;

	set TestString "Test FUNC_111: Restricted S Flash ";
	test_start $TestString;
	...
	test_compare $COULD_NOT_READ_VIA_ACCESS_PORT [IORap 0x1 [expr $SRAM_START_ADDR + ($SRAM0_SIZE/8*7) - 4]]; #This will cause openocd to crash. uncomment test and comment to test next
	test_end $TestString;

	set TestString "Test FUNC_111: CM7_0 AP Disabled ";
	test_start $TestString;
	test_compare $COULD_NOT_READ_VIA_ACCESS_PORT [IORap 0x2 [expr $SRAM_START_ADDR + ($SRAM0_SIZE/8*7) - 4] 0x0]; #This will cause openocd to crash. uncomment test and comment to test next
	test_end $TestString;

if {$DUT != $TVIIC2D4M_PSVP && $DUT != $TVIIC2D4M_SILICON} {
	set TestString "Test FUNC_111: CM7_1 AP Disabled ";
	test_start $TestString;
	test_compare $COULD_NOT_READ_VIA_ACCESS_PORT [IORap 0x3 [expr $SRAM_START_ADDR + ($SRAM0_SIZE/8*7) - 4]];;#This will cause openocd to crash. uncomment test and comment to test next
	test_end $TestString;
}


	set TestString "Test FUNC_111b: WriteRow on NAR ROW when NORMAL_ACCESS_CTL.WIDEN_ENABLE = 1 to Widen";
	test_start $TestString;
	...
	set TestString "Test FUNC_111c: Read NAR ";
	test_start $TestString;
	test_compare 0x0 [IOR [expr $SFLASH_START_ADDR + ($FLASH_ROW_SIZE*$SFLASH_NAR_ROW_IDX)]];
	test_end $TestString;

	set TestString "Test FUNC_111c: Read NDAR ";
	test_start $TestString;

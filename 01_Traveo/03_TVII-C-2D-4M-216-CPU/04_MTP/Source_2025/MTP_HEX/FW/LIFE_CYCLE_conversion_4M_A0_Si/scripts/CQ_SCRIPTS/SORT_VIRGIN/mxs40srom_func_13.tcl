...
set customer_fuse_array [list 0x68 0x69 0x6A 0x6B 0x6C];
set byte_size 0x8;
...
for {set iter 0} {$iter < $len_fuse_array} {incr iter} {
	...
	for {set bit_addr 0} {$bit_addr < $byte_size} {incr bit_addr} {
	    if {[expr $iter%0x2] == 0x00} {		
		    test_compare 0xA0000000 [SROM_BlowFuseBit $SYS_CALL_LESS32BIT $bit_addr $byte_addr_offset $macro_addr];
	    } else {
		    test_compare 0xA0000000 [SROM_BlowFuseBit $SYS_CALL_GREATER32BIT $bit_addr $byte_addr_offset $macro_addr];
		}
	}
}
...
for {set iter 0} {$iter < $len_fuse_array} {incr iter} {
	...
	set byte_addr [lindex $customer_fuse_array $iter];
	if {[expr $iter%2] == 0x00} {	
	    test_compare 0xA00000FF [SROM_ReadFuseByte $SYS_CALL_LESS32BIT $byte_addr];
    } else {
	    test_compare 0xA00000FF [SROM_ReadFuseByte $SYS_CALL_GREATER32BIT $byte_addr];
	}
}

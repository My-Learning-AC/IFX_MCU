...
while {$STATUS_SUCCESS != [SROM_CheckFmStatus $SYS_CALL_GREATER32BIT]} {
   puts "Waiting for FM Operation to complete";
}

set TestString "Test: EraseSector: Main Flash Large Sector";
test_start $TestString;
test_compare $STATUS_SUCCESS [SROM_EraseSector $SYS_CALL_GREATER32BIT $CM0P_NONBLOCKING $FLASH_START_ADDR $FM_INTR_MASK_RESET];
while {$STATUS_SUCCESS != [SROM_CheckFmStatus $SYS_CALL_GREATER32BIT]} {
   puts "Waiting for FM Operation to complete";
}
test_end $TestString;
...
test_start $TestString;
test_compare $STATUS_SUCCESS [SROM_ProgramRow $SYS_CALL_GREATER32BIT $CM0P_NONBLOCKING $SKIP_BLANK_CHECK_EN $DATA_WIDTH_4096BITS $DATA_LOC_SRAM $FM_INTR_MASK_RESET $FLASH_START_ADDR $userrow];
while {$STATUS_SUCCESS != [SROM_CheckFmStatus $SYS_CALL_GREATER32BIT]} {
   puts "Waiting for FM Operation to complete";
}
test_end $TestString;
...
test_start $TestString;
test_compare $STATUS_SUCCESS [SROM_ProgramRow $SYS_CALL_GREATER32BIT $CM0P_NONBLOCKING $SKIP_BLANK_CHECK_EN $DATA_WIDTH_256BITS $DATA_LOC_SRAM $FM_INTR_MASK_RESET $FLASH_START_ADDR $userrow];
while {$STATUS_SUCCESS != [SROM_CheckFmStatus $SYS_CALL_GREATER32BIT]} {
   puts "Waiting for FM Operation to complete";
}
test_end $TestString;
...
test_start $TestString;
test_compare $STATUS_SUCCESS [SROM_EraseSector $SYS_CALL_GREATER32BIT $CM0P_NONBLOCKING $FLASH_START_ADDR $FM_INTR_MASK_RESET];
while {$STATUS_SUCCESS != [SROM_CheckFmStatus $SYS_CALL_LESS32BIT]} {
   puts "Waiting for FM Operation to complete";
}
test_end $TestString;
...
test_start $TestString;
test_compare $STATUS_SUCCESS [SROM_ProgramRow $SYS_CALL_GREATER32BIT $CM0P_NONBLOCKING $SKIP_BLANK_CHECK_EN $DATA_WIDTH_64BITS $DATA_LOC_SRAM $FM_INTR_MASK_RESET $FLASH_START_ADDR $userrow];
while {$STATUS_SUCCESS != [SROM_CheckFmStatus $SYS_CALL_LESS32BIT]} {
   puts "Waiting for FM Operation to complete";
}
test_end $TestString;
...
test_start $TestString;
set rowID 0x0;
test_compare $STATUS_SUCCESS [SROM_Checksum $SYS_CALL_LESS32BIT $rowID $PAGE $FLASH_REGION_MAIN $BANK0];
test_end $TestString;
...
for {set data_width $DATA_WIDTH_32BITS} {$data_width <= $DATA_WIDTH_32BITS} { incr data_width} {
		...
		test_start $TestString;
		test_compare $STATUS_SUCCESS [SROM_EraseSector $SYS_CALL_GREATER32BIT $CM0P_NONBLOCKING $WFLASH_START_ADDR $FM_INTR_MASK_RESET];
		while {$STATUS_SUCCESS != [SROM_CheckFmStatus $SYS_CALL_GREATER32BIT]} {
           puts "Waiting for FM Operation to complete";
        }
		test_end $TestString;
		...
		test_start $TestString;
		test_compare $STATUS_SUCCESS [SROM_ProgramRow $SYS_CALL_GREATER32BIT $CM0P_NONBLOCKING $SKIP_BLANK_CHECK_EN $data_width $DATA_LOC_SRAM $FM_INTR_MASK_RESET $WFLASH_START_ADDR $userrow];
		while {$STATUS_SUCCESS != [SROM_CheckFmStatus $SYS_CALL_GREATER32BIT]} {
		   puts "Waiting for FM Operation to complete";
		}
		test_end $TestString;

...
int main(void)
{
    ...
    printf("ins: CM7_0 Initialization finished...\n");
    ...
    writeStatusToFlash(MIPI_E, false);


    /* Draw something. */
    ret = BufferedCapture_MIPI_Init();

    printf("log: MIPI Camera Test\n");
    printf("ins: -- Connect the jumpers R736,R737, R739, R740 and R741\n");
    printf("ins: Press SW3 button if image is showing on display. Otherwise press SW6\n");
    ...
    if (readStatusFromFlash(MIPI_E) == true )
    {
        printf("log: MIPI Camera working successfully\n");
    }
    else 
    {
        printf("log: MIPI Camera is not working\n");
    }
...
static void ButtonCallback(uint8_t u8ButtonId, cy_button_en_state_t enState)
{
    ...
    switch (u8ButtonId)
    {
        ...
        case CY_GFXENV_BUTTON_ID_LEFT:
            writeStatusToFlash(MIPI_E, true);
            bRunning = CYGFX_FALSE;
            break;

...
#include "mtp\tvii_series_smif_ex_adopter_hram.h"
#include "mtp\hram_TestData.h"

/* User defined header file for HyperRAM */
...
bool hram_test(void)
{
    ...
    if(readVCRValue0 != ramVCR.u16) return false;
    ...
    uint32_t targetSmifFreq = 32000000;
    ...
    // Verify data
    if(memcmp((void*)readBuf, (void*)programData, TEST_PROGRAM_SIZE) != 0) return false;
    ...
    for(uint32_t i_WriteAddr = 0x00000000ul; i_WriteAddr < (TEST_PROGRAM_SIZE / 2);i_WriteAddr += 0x100)
    {
        ...
        if(memcmp(readBuffer, &programData[i_WriteAddr*2], 0x200) != 0) return false;
    }
    Cy_SysTick_DelayInUs(1000000);                // For convinient printing "successful working" message into MTP GUI

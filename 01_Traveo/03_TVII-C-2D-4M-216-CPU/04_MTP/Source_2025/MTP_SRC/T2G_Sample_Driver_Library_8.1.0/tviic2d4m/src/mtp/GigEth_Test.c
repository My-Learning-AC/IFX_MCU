...
extern bool Timeout;
uint16_t frameType = 0x22F0;     // This frame type is present in the TestJig Ethernet code side, so don't change it
volatile uint16_t EthType = 0;
bool EthernetTest = false;
...
const uint8_t gMAC_SRC_ADDR[6] = {0xBB,0xBB,0xBB,0xBB,0xBB,0xBB};
...
bool EthTest(void)
{
    ...
    for(;;)
    {
      if(TransmitCount < 7)
      {
          Cy_EthIf_TransmitTrigger(ETH_REG_BASE, bufferIdx, sizeof(message), qIdx, frameType, (uint8_t*)gMAC_DST_ADDR);
          Cy_SysTick_DelayInUs(50000);
          TransmitCount++;
      }
      
      if(EthernetTest == true)
      {
        EthernetTest = false;           // for making the test as false
        return true;
      }
      else
      {
...
static void Ethx_RxFrameCB(cy_pstc_eth_type_t pstcEth, uint8_t * u8RxBuffer, uint32_t u32Length)
{
    ...
    if((EthType == frameType) && (ReceiveCount < 7))
    {
      EthernetTest = true;
      ReceiveCount++;
    }

...
#include "tvii_series_smif_ex_adopter_hflash.h"
#include "hflash_TestData.h"

/* User defined header file for HyperFlash */
#include "smif_hflash.h"

/************ Modifiable definitions **********/
#define TEST_SECTOR_NO          (5)            // modifiable 0 ~ 255
...
#define TARGET_CLK_FREQ_IN_MHZ  (166)
/**********************************************/
...
cy_stc_smif_config_t const smifhflashConfig =
{
...
cy_stc_smif_context_t smifhflashContext;

#define HB_FLASH_TARGET_LC  CY_SMIF_HB_LC14     //CY_SMIF_HB_LC20
...
cy_stc_smif_port_t smifhflashPortCfg[] =
{
...
#define SIZE_OF_PORT (sizeof(smifhflashPortCfg)/sizeof(cy_stc_smif_port_t))
...
void CalibrateDelayLineSelect1(uint32_t freq, bool isDDR);
...
cy_stc_mpu_region_cfg_t g_mpuhflashCfg[] =
{
...
void hflash_init(void)
{
    ...
    Cy_MPU_Setup(g_mpuhflashCfg, MPU_CFG_ARRAY_SIZE(g_mpuhflashCfg), CY_MPU_DISABLE_USE_DEFAULT_MAP, CY_MPU_DISABLED_DURING_FAULT_NMI);

    ChangePLLFrequency(64000000); // SMIF out clock will be 32,000,000
    Cy_SysClk_HfClkEnable(SMIF_HF_CLOCK);   // 32MHz

    SmifPortInit(smifhflashPortCfg ,SIZE_OF_PORT);
    ...
    Cy_SMIF_Init(SMIF_USED, &smifhflashConfig, 1000, &smifhflashContext);
    Cy_SMIF_Set_DelayTapSel(SMIF_USED, (CY_SMIF_GetDelayTapsNumber(SMIF_USED) - 1));
    Cy_SMIF_Enable(SMIF_USED, &smifhflashContext);
...
bool hflash_test(void)
{
    ...
    if(readConfig1RegVal != flashVCR1.u16) return false;
    ...
    if(readConfig2RegVal != flashVCR2.u16) return false;
    ...
    CalibrateDelayLineSelect1(targetSmifFreqFlash, true);
    ...
    for(uint32_t i_WriteAddr = 0; i_WriteAddr < (TEST_PROGRAM_SIZE / 2);i_WriteAddr += HB_WRITE_BUF_SIZE_WORD)
    {
        ...
        CY_SMIF_HbFlash_WriteBufferProgramOp(pHyperFlashBaseAddr, (i_WriteAddr+TEST_SECTOR_ADDRESS), HB_WRITE_BUF_SIZE_WORD, (CY_SMIF_FLASHDATA*)&programData1[i_WriteAddr*2]);
    }
    ...
    g_mpuhflashCfg[SMIF_MPU_REG_NO].attribute = CY_MPU_ATTR_NORM_MEM_WT;
    if(Cy_MPU_Setup(g_mpuhflashCfg, MPU_CFG_ARRAY_SIZE(g_mpuhflashCfg), CY_MPU_DISABLE_USE_DEFAULT_MAP, CY_MPU_DISABLED_DURING_FAULT_NMI) != CY_MPU_SUCCESS) return false;

    /*********  Read back and verify   *******/
    static uint8_t readBuf[TEST_PROGRAM_SIZE];
    memcpy(readBuf, (void*)(pHyperFlashBaseAddr + TEST_SECTOR_ADDRESS), TEST_PROGRAM_SIZE);

    // Verify data
    if(memcmp(readBuf, programData1, TEST_PROGRAM_SIZE) != 0) return false; 
...
{
    Cy_SMIF_SEMP_WriteEnable(SMIF_USED, slave, transMode, &smifhflashContext);
    Cy_SMIF_SEMP_WriteAnyRegister(SMIF_USED, slave, value, addr, addrLen, transMode, &smifhflashContext);
...
static bool DetectVolatileRegLantency(cy_en_smif_slave_select_t slave, cy_en_smif_semp_reg_latency_code_t* detectRegLatency)
{
    ...
    Cy_SMIF_SEMP_ReadId(SMIF_USED, slave, &readId, CY_SMIF_SEMP_REG_LATENCY0, CY_SPI_TRANSACTION_1S1S1S, sizeof(cy_stc_smif_semp_id_t), &smifhflashContext);
...
static void EnterHyperBusMode(volatile cy_stc_smif_reg_device_t *device)
{
    ...
    /*************************/
    Cy_SMIF_DeInit(SMIF_USED);
    Cy_SMIF_Init(SMIF_USED, &smifSpiConfig, 1000, &smifhflashContext);

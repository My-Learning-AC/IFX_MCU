...
#define EthernetTypeMatch       0xF022
volatile uint16_t EthType = 0;

void GigEth_Init(void);
...
const cy_stc_ethif_interruptconfig_t stcInterruptConfig = {
    ...
    .btx_complete           = false,  /** Frame has been transmitted successfully */
    ...
    .txerrorcb      = NULL,
    .txcompletecb   = NULL, //Ethx_TxFrameSuccessful, /** Set it to NULL, if do not wish to have callback */
    .tsuSecondInccb = NULL, //Ethx_TSUIncrement,
    .tsuCompMatchcb = NULL,
};
...
const uint8_t gMAC_DST_ADDR[6] = {0xFF,0xFF,0xFF,0xFF,0xFF,0xFF};
  ...
  Cy_SysClk_HfClkEnable(CY_SYSCLK_HFCLK_2);
    ...
    /* Initialize LIN */
    {
		stc_lin_config_t lin_config =
        {
            ...
            .u8BreakFieldLength = 13u, //Break field length
    ...
    else
    {
        ...
        case LIN_STATE_RX_HEADER:
            ...
            if (acceptedId)
            {
                ...
                {
                    msgContext[currentMsgIdx].dataBuffer[0] = 0x12;
                    msgContext[currentMsgIdx].dataBuffer[1] = 0x34;
                    msgContext[currentMsgIdx].dataBuffer[2] = 0x56;
                    msgContext[currentMsgIdx].dataBuffer[3] = 0x78;
                    msgContext[currentMsgIdx].dataBuffer[4] = 0x90;
                    msgContext[currentMsgIdx].dataBuffer[5] = 0xAB;
                    msgContext[currentMsgIdx].dataBuffer[6] = 0xCD;
                    msgContext[currentMsgIdx].dataBuffer[7] = 0xEF;
...
static void Ethx_RxFrameCB(cy_pstc_eth_type_t pstcEth, uint8_t * u8RxBuffer, uint32_t u32Length)
{
    ...
    EthType = (uint16_t)pRxPacket->frameType;
    
    if((EthType == EthernetTypeMatch) && (TransmitCount < 7))
    {
        /* Get a pointer to the tx buffer which has been allocated the driver */
        uint8_t bufferIdx;
        uint8_t* pTxBuffer;
        ...
        /* Copy an original message to the top of the tx buffer */
        int8_t message[] = "TVIIC2D4M sent back a received message: ";
...
static void InitPHY_DP83848Q_Q1(void)
{
    ...
    wr_bmcr.f.u1AUTO_NEGOTIATION_ENABLE = 1u;                        // Auto-negotiation on

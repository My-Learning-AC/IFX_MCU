...
void IfxXspi_Spi_enhancedDataTransmit(Ifx_XSPI *xspi, IfxXspi_Spi_EnhancedModeConfig *config)
{
    uint16 count = config->numberOfDataFrames, index = 0;
    ...
    while (count != 0)
    {
        xspi->DR[0].U = (uint32)(config->txData[index++]);
        count         = count - 1;
    }

    while ((xspi->SR.B.BUSY == 1) || (xspi->SR.B.TFE == 0))
    {}
}
...
{
    uint16         count  = config->numberOfDataFrames, tx_index = 0;
    IfxXspi_Status status = IfxXspi_Status_success;
    ...
    while (count != 0)
    {
        xspi->DR[0].U = (uint32)(config->txData[tx_index++]);
        count         = count - 1;
    }

    while ((xspi->SR.B.BUSY == 1) || (xspi->SR.B.TFE == 0) || (xspi->SR.B.RFNE == 0))
    {}

    if (xspi->SR.B.RFNE == 1)
    {
        uint32 rx_index = 0, rx_count = (uint32)(xspi->RXFLR.B.RXTFL);            // get the readable count of Rx fifo

        if (rx_count == 0)
        {
            status = IfxXspi_Status_failure;
        }
...
void IfxXspi_Spi_setXspiGpioPins(Ifx_XSPI *xspi, IfxXspi_Spi_GpioPins *pins)
{
    ...
    xspi->CTRLR2.B.PORTSEL = 1;    // selects GPIO pad as xSPI data, CS, CLK pins
    /*

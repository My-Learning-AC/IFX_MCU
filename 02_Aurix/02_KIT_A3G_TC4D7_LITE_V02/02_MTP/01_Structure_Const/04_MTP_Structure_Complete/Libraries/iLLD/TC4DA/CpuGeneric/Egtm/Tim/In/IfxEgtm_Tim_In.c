...
boolean IfxEgtm_Tim_In_init(IfxEgtm_Tim_In *driver, const IfxEgtm_Tim_In_Config *config)
{
    ...
    case IfxEgtm_Tim_In_Src_channelCurrentOrPrevious:
        ...
        config->egtm->CLS[clsIndex].TIM.IN_SRC.U = ((uint32)1u << (IFX_EGTM_CLS_TIM_IN_SRC_MODE0_OFF + (uint8)channelIndex * (IFX_EGTM_CLS_TIM_IN_SRC_MODE1_OFF - IFX_EGTM_CLS_TIM_IN_SRC_MODE0_OFF)));                // MODEx 0th bit is set but the 1st bit = 0, so MODEx[1:1]=0
        config->egtm->CLS[clsIndex].TIM.IN_SRC.U = ((uint32)1u << (IFX_EGTM_CLS_TIM_IN_SRC_VAL0_OFF + (uint8)channelIndex * (IFX_EGTM_CLS_TIM_IN_SRC_VAL1_OFF - IFX_EGTM_CLS_TIM_IN_SRC_VAL0_OFF)));                   // VALx 0th bit is set but the 1st bit = 0, so VALx[1:1]=0

        if (srcTimIn == IfxEgtm_Tim_In_SrcTimIn_currentChannel)
        {
            config->egtm->CLS[clsIndex].TIM.CH[channelIndex].CTRL.U = (uint32)0u << IFX_EGTM_CLS_TIM_CH_CTRL_CICTRL_OFF;                                                                                                   //CICTRL = 0
            channel->CTRL.B.CICTRL                                  = (uint8)IfxEgtm_Tim_In_SrcTimIn_currentChannel;
        }
        else
        {
            config->egtm->CLS[clsIndex].TIM.CH[channelIndex].CTRL.U = (uint32)1u << IFX_EGTM_CLS_TIM_CH_CTRL_CICTRL_OFF;                                                                                                   //CICTRL = 1
            channel->CTRL.B.CICTRL                                  = (uint8)IfxEgtm_Tim_In_SrcTimIn_previousChannel;
        }
    ...
    case IfxEgtm_Tim_In_Src_auxiliary:
        config->egtm->CLS[clsIndex].TIM.IN_SRC.U = ((uint32)1u << (IFX_EGTM_CLS_TIM_IN_SRC_MODE0_OFF + (uint8)channelIndex * (IFX_EGTM_CLS_TIM_IN_SRC_MODE1_OFF - IFX_EGTM_CLS_TIM_IN_SRC_MODE0_OFF)));  // MODEx 0th bit is set but the 1st bit = 0, so MODEx[1:1]=0
        config->egtm->CLS[clsIndex].TIM.IN_SRC.U = ((uint32)2u << (IFX_EGTM_CLS_TIM_IN_SRC_VAL0_OFF + (uint8)channelIndex * (IFX_EGTM_CLS_TIM_IN_SRC_VAL1_OFF - IFX_EGTM_CLS_TIM_IN_SRC_VAL0_OFF)));     // VALx 1st bit = 1, so VALx[1:1]=1
        break;
    ...
    default:
        ...
        config->egtm->CLS[clsIndex].TIM.IN_SRC.U                = ((uint32)1u << (IFX_EGTM_CLS_TIM_IN_SRC_MODE0_OFF + (uint8)channelIndex * (IFX_EGTM_CLS_TIM_IN_SRC_MODE1_OFF - IFX_EGTM_CLS_TIM_IN_SRC_MODE0_OFF))); // MODEx 0th bit is set but the 1st bit = 0, so MODEx[1:1]=0
        config->egtm->CLS[clsIndex].TIM.IN_SRC.U                = ((uint32)1u << (IFX_EGTM_CLS_TIM_IN_SRC_VAL0_OFF + (uint8)channelIndex * (IFX_EGTM_CLS_TIM_IN_SRC_VAL1_OFF - IFX_EGTM_CLS_TIM_IN_SRC_VAL0_OFF)));    // VALx 0th bit is set but the 1st bit = 0, so VALx[1:1]=0
        config->egtm->CLS[clsIndex].TIM.CH[channelIndex].CTRL.U = (uint32)0u << IFX_EGTM_CLS_TIM_CH_CTRL_CICTRL_OFF;
        channel->CTRL.B.CICTRL                                  = (uint8)IfxEgtm_Tim_In_SrcTimIn_currentChannel;
        break;
    }
    ...
    {
        config->egtm->CLS[clsIndex].TIM.CH[channelIndex].ECTRL.U = config->egtm->CLS[clsIndex].TIM.CH[channelIndex].ECTRL.U & 0xFF3FFFFF; //Clearing the LUT bits to make sure is not configured.
    }
    else
    {
        //for LUT_IN2
        ...
        case IfxEgtm_Tim_In_SrcForLutIn2_external_capture:
            config->egtm->CLS[clsIndex].TIM.CH[channelIndex].ECTRL.U = (uint32)1u << IFX_EGTM_CLS_TIM_CH_ECTRL_USE_LUT_OFF;                  //setting ECTRL 23rd bit=0 22nd bit = 1
            break;

        case IfxEgtm_Tim_In_SrcForLutIn2_inputSignal_previousChannel:
            config->egtm->CLS[clsIndex].TIM.CH[channelIndex].ECTRL.U = (uint32)2u << IFX_EGTM_CLS_TIM_CH_ECTRL_USE_LUT_OFF;                  //setting ECTRL 23rd bit=1 22nd bit = 0
            break;
        case IfxEgtm_Tim_In_SrcForLutIn2_outputSignal_tssmMode:
            config->egtm->CLS[clsIndex].TIM.CH[channelIndex].ECTRL.U = (uint32)3u << IFX_EGTM_CLS_TIM_CH_ECTRL_USE_LUT_OFF;                  //setting ECTRL 23rd bit=1 22nd bit = 1
            break;
        default:
